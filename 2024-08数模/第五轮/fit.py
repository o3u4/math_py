...
# params1, params2 = fit_pdf(x, y_male, y_female)
# c_fit1, a_fit1, b_fit1 = params1
# c_fit2, a_fit2, b_fit2 = params2
# # 打印拟合的参数
# print(f"拟合的参数: c1 = {c_fit1}, a1 = {a_fit1}, b1 = {b_fit1}")
# print(f"拟合的参数: c2 = {c_fit2}, a2 = {a_fit2}, b2 = {b_fit2}")

params1, params2 = fit_pdf(x, y_male, y_female)
c_fit1, a_fit1, d_fit1, p_fit1 = params1
c_fit2, a_fit2, d_fit2, p_fit2 = params2
# 打印拟合的参数
...
# plt.plot(x_, generalized_gamma_pdf(x_, *params1), label='男(gamma拟合)', color='#509caf')
# plt.plot(x_, generalized_gamma_pdf(x_, *params2), label='女(gamma拟合)', color='#8a72a5')

# 设置图例和标签
plt.xticks(x[::3], [int(i * 100 / 21) for i in x[::3]], rotation=0)  # 每隔一个标签显示一次  # 设置x轴为年龄段标签
plt.xlabel('年龄段')
plt.ylabel('人口数')
plt.title('不同年龄段的男女死亡人口数')
plt.legend()
plt.grid(True)
